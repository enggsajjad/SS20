...
// Generated by ASIP Meister 2.3 on 2020/06/18 18:27:17 
Design{
Version{"2.3"},
...
"Operand"{"name"{"rs1"},width{"31","27"}},
"Operand"{"name"{"rs2"},width{"26","22"}},
"Operand"{"name"{"rs3"},width{"21","17"}},
"Operand"{"name"{"rd"},width{"16","12"}},
"OP-code"{"binary"{"000"},width{"11","9"}},
"OP-code"{"binary"{"000"},width{"8","6"}},
...
"OP-code"{"binary"{"00010"},width{"10","6"}},
"OP-code"{"binary"{"000000"},width{"5","0"}}
},"CLAMPV"{type{"RT"},"Operand"{"name"{"rd"},width{"31","27"}},
"Operand"{"name"{"rs1"},width{"26","22"}},
"OP-code"{"binary"{"00000000010"},width{"21","11"}},
"OP-code"{"binary"{"00010"},width{"10","6"}},
"OP-code"{"binary"{"000000"},width{"5","0"}}
},"CLAMPI"{type{"RT"},"Operand"{"name"{"rd"},width{"31","27"}},
"Operand"{"name"{"rs1"},width{"26","22"}},
"OP-code"{"binary"{"00000000011"},width{"21","11"}},
"OP-code"{"binary"{"00010"},width{"10","6"}},
"OP-code"{"binary"{"000000"},width{"5","0"}}
},"VALPRED"{type{"RR"},"Operand"{"name"{"rs1"},width{"31","27"}},
"Operand"{"name"{"rs2"},width{"26","22"}},
"Operand"{"name"{"rd"},width{"21","17"}},
...
bit_width{"32"},
algorithm{"default"}}}
,"ALU2"{class{"alu"},classpath{"/basicfhmdb/computational/"},use{"(unspecified)"},
parameter{
abstraction_level{for_simulation{"Behavior"},for_synthesis{"Gate"}},
bit_width{"32"},
algorithm{"default"}}}
,"ALU3"{class{"alu"},classpath{"/basicfhmdb/computational/"},use{"(unspecified)"},
parameter{
abstraction_level{for_simulation{"Behavior"},for_synthesis{"Gate"}},
bit_width{"32"},
algorithm{"default"}}}
...
,"CLAMPV"{variable{"wire [31:0] result;
/* these variables can be used in following stages of all instructions */
wire [31:0] source1;"},clk(1){"wire [31:0]  fetch_current_pc;                              // current pc
wire [31:0]  fetch_inst;                                    // opecode
wire         fetch_addrerr;                                 // address err. from IM
...
gpr_read_stmp1 = GPR.read0(rs1);
source1        = FWU0.forward(rs1, gpr_read_stmp1);"},
clk(3){"wire [3:0] alu_cmp_alu_flag;
wire [1:0] alu_cmp_tmp_flag;
wire       alu_cmp_lt_cond1;
wire       alu_cmp_lt_cond2;
wire       clamp_low;

wire [3:0] alu_cmp_alu_flag2;
wire [1:0] alu_cmp_tmp_flag2;
wire       alu_cmp_lt_cond12;
wire       alu_cmp_lt_cond22;
wire       clamp_high;

wire [31:0] lower_bound;
wire [31:0] upper_bound;

wire [31:0] clamped_value;
wire        clamped;

lower_bound = \"11111111111111111000000000000000\";
upper_bound = \"00000000000000000111111111111111\";



alu_cmp_alu_flag = ALU.cmp(source1, lower_bound);
...
alu_cmp_lt_cond1   = alu_cmp_tmp_flag == \"10\";
alu_cmp_lt_cond2   = alu_cmp_tmp_flag == \"01\";
clamp_low          = alu_cmp_lt_cond1 | alu_cmp_lt_cond2;

alu_cmp_alu_flag2 = ALU1.cmp(upper_bound, source1);

// signed less than is either sign bit is 1 with no overflow
//                     or     sign bit is 0 with overflow
alu_cmp_tmp_flag2   = alu_cmp_alu_flag2[1:0];
alu_cmp_lt_cond12   = alu_cmp_tmp_flag2 == \"10\";
alu_cmp_lt_cond22   = alu_cmp_tmp_flag2 == \"01\";
clamp_high          = alu_cmp_lt_cond12 | alu_cmp_lt_cond22;

clamped = clamp_low | clamp_high;

clamped_value = (clamp_low) ? lower_bound:upper_bound;

result = (clamped) ? clamped_value:source1;
// foward result to as GPR[rd] from stage EXE
null = FWU0.forward1(rd, result);
null = FWU1.forward1(rd, result);"},
clk(4){"null = GPR.write0(rd, result);        // write result[arg2] to GPR[arg1]
// foward result to as GPR[rd] from stage WB
null = FWU0.forward2(rd, result);
null = FWU1.forward2(rd, result);"}
}
,"CLAMPI"{variable{"wire [31:0] result;
/* these variables can be used in following stages of all instructions */
wire [31:0] source1;"},clk(1){"wire [31:0]  fetch_current_pc;                              // current pc
wire [31:0]  fetch_inst;                                    // opecode
wire         fetch_addrerr;                                 // address err. from IM
...
lower_bound = \"00000000000000000000000000000000\";
upper_bound = \"00000000000000000000000001011000\";
...
,"VALPRED"{variable{"wire [31:0] result;
/* these variables can be used in following stages of all instructions */
wire [31:0]  source1;
wire [31:0]  source2;"},clk(1){"wire [31:0]  fetch_current_pc;                              // current pc
wire [31:0]  fetch_inst;                                    // opecode
...
zero17 = \"00000000000000000\";
zero18 = \"000000000000000000\";
zero19 = \"0000000000000000000\";
step_tmp1 = source1[31:16];
step_tmp2 = source1[31:17];
step_tmp3 = source1[31:18];
step_tmp4 = source1[31:19];
step =    <zero16, step_tmp1>;
step_1s = <zero17, step_tmp2>;
step_2s = <zero18, step_tmp3>;
step_3s = <zero19, step_tmp4>;
delta3 = delta[2];
delta2 = delta[1];
delta1 = delta[0];


vpdiff_1 = (delta3) ? step:zero;
vpdiff_2 = (delta2) ? step_1s:zero;
vpdiff_3 = (delta1) ? step_2s:zero;

<temp1, flag1> = ALU.add(vpdiff_1, vpdiff_2);
<temp2, flag2> = ALU1.add(vpdiff_3, step_3s);
<temp3, flag3> = ALU2.add(temp1, temp2);
not_temp3 = ~temp3;
<temp4, flag4> = ALU3.add(one, not_temp3);
